...
the space of infinitely differentiable functions in $\Omega$. For $f\in\mathcal{C}^\infty(\Omega)$
\begin{IEEEeqnarray*}{rCcCl}
    \partial^{\alpha}f&=&f^{(\alpha)}&=&(\frac{\partial}{\partial\bx})^{\alpha}f.
\end{IEEEeqnarray*}
Let $\mathcal{C}_0^\infty(\Omega)$ denote the vectorial subspace of $\mathcal{C}^\infty(\Omega)$
functions that have compact support
\noindent{\color{blue}\#\#\#\#\#\#\# continue here: dup scott page 444.} 
\begin{defi}
\begin{IEEEeqnarray*}{rCl}
...
\section{Polynomial Interpolation} % (fold)
...
% section polynomial_interpolation (end)
