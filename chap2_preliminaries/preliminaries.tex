...
\begin{defi}
  \begin{IEEEeqnarray*}{rCl}
    H(\curl,\Omega) & := & \{\bu\in L^2(\Omega)^3\,:\,\curl \bu \in L^2(\Omega)^3\} \\[5pt]
    \|\bu\|_{H(\curl,\Omega)} & := & \left(\|\bu\|^2_{L^2(\Omega)^3}+
      \|\dv \bu\|^2_{L^2(\Omega)^3}\right)^{\nicefrac12}.
  \end{IEEEeqnarray*}
\end{defi}
...
\end{theorem}
With regard to trace properties in $H(  )$
...
\begin{IEEEeqnarray*}{rCl}
	\bu & = &
	  ...
	  \end{cases}.
\end{IEEEeqnarray*}
Then $\bu$ is in $H(\curl,\Omega)$ if and only if
...
%$H(\curl)$--conforming if every time we take two
%\emph{push-forward} elements $(E_1, P_{E_1}, \Sigma_1)$
%and $(E_2, P_{E_2}, \Sigma_2)$ according
...
%   \end{cases}
%\end{IEEEeqnarray*}
%results in $H(\curl,\Omega_1\cup\Omega_2)$.
%\end{defi}
