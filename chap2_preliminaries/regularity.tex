...
at those vertices or approaching those edges.\\[5pt]
...
We also define *********** SEGUIR ACA CON pag. 1119 de ALW\\[5pt]

$R(\bxi)$ as the distance from $\bxi$ to $S$,
$r(\bx)$ will be the \emph{radial} distance from $\bx$ to $A_{S,j}$ and $\theta(\bx)$ will be
the \textsl{angular distance} $\theta(\bx)=\frac{r(\bx)}{R(\bx)}$.
