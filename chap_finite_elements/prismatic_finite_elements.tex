...
Given $p>2$, we define the $H(\bcurl)$--conforming interpolation operator on the reference
...
\begin{IEEEeqnarray*}{rCl}
    \yesnumber\label{dk}
    D_k & = & P_{k-1}^2(x,y) \oplus \widetilde{P}_{k-1}(x,y) \bx,\\
    \bx & = & (x,y).
\end{IEEEeqnarray*}
...
\begin{proof}
    ...
    \begin{IEEEeqnarray*}{rCl}
        \dim (P_{k-1}^2(x,y) \oplus \widetilde{P}_{k-1}(x,y)) \otimes P_{k-1}(z) + \dim P_{k-1}(x,y)\otimes P_k(z) & = &\\[5pt]
