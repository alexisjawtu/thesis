...
\begin{center}
        ...
        \end{tikzpicture}
    \end{center}
where $V_{\hat E}$, $W_{\hat E}$ and $P_{\hat E}$ are~(\ref{spaceFEprismHcurl}),~(\ref{prismaticSpace})
...
let $\hat\btau$, $\btau$, $\hat\bn$ and $\bn$ be tangents and normals in
...
\begin{IEEEeqnarray}{ll}
    ...
    \varphi_{f,\bq}\,(\bu) =  
    \int_{f} \bu \cdot \bq_0\,
    dS\mbox{, } &\bq_0 = (\det M_E\|M_E^{-t}\hat\bn\|)^{-1} M_E\hat{\bq}_T\\
    &\hat{\bq}_T := (\hat\bn\times\hat\bq)\times\hat\bn\\
...
\begin{corollary}
 \label{aux_label26} 
  For any $\bu\in W^{1,p}(E)$ expressions 
...
\end{proof}
\begin{corollary}
  Finite Element on $E$ is $H(\Div)$--conforming and unisolvent.
\end{corollary}
\begin{corollary}
