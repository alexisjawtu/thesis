...
\begin{IEEEeqnarraybox*}{lCl}
  h_1, h_2 &\sim&
    \begin{cases}
      \textit{h}^{\frac{1}{\mu}}  & \text{ if\, }d(E,e)=0\\
      \textit{h}\,d(E,e)^{1-\mu}  & \text{ if\, }0<d(E,e)\lesssim1\\
      \textit{h}          & \text{ if\, }d(E,e)\sim1
    \end{cases}\\[5pt]
  h_3   &\sim& 
    \begin{cases}
      \textit{h}^{\frac{1}{\nu}}  & \text{ if\, }d(E,v)=0\\
...
$d(E,\be) = 0$.
\begin{IEEEeqnarray*}{rCl}
  \| \bu_s - \boldsymbol{r}\bu _s\|_{L^2(\Lambda_\ell)}^2 &\leqslant&
  \sum_{d(K,e) = 0} \left(\| \bu_s\|_{L^2(E)} + 
  \|\boldsymbol{r}\bu _s\|_{L^2(E)}\right)^2\\
  & &\:+\sum_{d(K,e) > 0} \left( \sum_{|\alpha| = 1} 
  \boldsymbol{h}^\alpha \|D^\alpha\bu_s\|_{L^2(E)} + 
  h_K\|\text{div}\bu_s\|_{L^2(E)}
  \right)^2.\\
...
\paragraph{Elements with $d(E,e) > 0$.} % (fold)
...
\end{IEEEeqnarray*}
Let's look for $\delta$. We have $r(\bx)\geqslant d(E,e)$ so, according to the
grading~(\ref{label_grading}) , in the 
case $0< d(E,e) <1$, it holds
\begin{IEEEeqnarray*}{rClCl}
  \textit{h}\,r(\bx)^{1-\mu}&\geqslant&\textit{h}\,d(E,e)^{1-\mu}&\sim& h_1.
\end{IEEEeqnarray*}
In the 
case $d(E,e) \sim 1$
\begin{IEEEeqnarray*}{rClCl}
...
\item { \noindent{\color{BrickRed}\#\#\#\#\#\#\# continue here}  Bound for $\|\partial_{x_3}\bu_s\|_{L^2(E)}$.} % (fold)
\label{subp:bound_for_001}
If $i = 1,2,3$, $\partial_{x_3}u_i$ equals $\partial_{x_i}u_3$.
\begin{IEEEeqnarray*}{rCl}
  h_3\|\partial_{x_3}\bu_s\|_{L^2(E)}& = & 
    \sum_{i=1,2,3} h_3\,\|\partial_{x_3}u_{s,i}\|_{L^2(E)}\\
    &\lesssim&\textit{h}\,\| u_3 \|_{V_0^{1,2}(\Lambda_\ell)}\leqslant\textit{h}\,\|f\|_{L^2(\Lambda_\ell)}.
\end{IEEEeqnarray*}
\item {Bound for the divergence.} % (fold)
...
\subparagraph{Bound for $\| \bu_s\|_{L^2(E)}$.} % (fold)
\label{subp:bound_for_sigma_i}
\begin{IEEEeqnarray*}{rCl}
  \|\bu_s\|_{L^2(E)}& = & \sum_{i=1,2,3}\|{u_{s,i}}\|_{L^2(E)}\\
  &=&\sum_{i=1,2,3}\|r(\bx)^{1-\delta} r(\bx)^{\delta-1} {u_{s,i}}\|_{L^2(E)}\\
...
\subparagraph{Bound for $\| \boldsymbol{r} \bu_s \|_{L^{2}(K)}$.} % (fold)
...
\begin{IEEEeqnarray*}{rCl}
  \| (\boldsymbol{r}\bu_s)_1 \|_{L^{1}(K)}  & = & \int\limits_K |(\boldsymbol{r} \bu_s)_1|\,d\bx\\
        & = & \int\limits_{\tilde{K}} |(A\,\tilde{\boldsymbol{r}}\tilde{\bu}_s)_1|\,d\tilde{\bx}\\
        & \leqslant & {\color{BrickRed} \|A\|_\infty} \left[\|\tilde{u}_{s,1}\|_{L^1(\tilde{K})} + \|\tilde{u}_{s,2}\|_{L^1(\tilde{K})}\right.\\
   &    & +\;h_1 \left( \| \partial_{x} \tilde{u}_{s,1} \|_{L^{1}(\tilde{K})} +
        \| \partial_{x} \tilde{u}_{s,2} \|_{L^{1}(\tilde{K})}\right)\\
   &    & +\;h_2 \left( \| \partial_{y} \tilde{u}_{s,1} \|_{L^{1}(\tilde{K})} +
        \| \partial_{y} \tilde{u}_{s,2} \|_{L^{1}(\tilde{K})}\right)\\
   &    & +\;\left.h_3 \left( \| \partial_{z} \tilde{u}_{s,1} \|_{L^{1}(\tilde{K})} +
        \| \partial_{z} \tilde{u}_{s,2} \|_{L^{1}(\tilde{K})}\right) + 
          2 h_1\|\tilde{\text{div}}(\tilde{u}_{s,1},\tilde{u}_{s,2},0)\|_{L^{1}(\tilde{K})}\right].
\end{IEEEeqnarray*}
...
\begin{IEEEeqnarray*}{rCl}
  \|\tilde{u}_{s,1}\|_{L^1(\tilde{K})} & = &
  \frac{1}{|\det(A)|} \int\limits_{K} \left|\tilde{u}_{s,1}(F^{-1}(\bx))\right|\,d\bx\\
        & =   &         \int\limits_{K} |A^{-1}_{\text{row}_1}\bu_s(\bx)|\,d\bx\\
        & \leqslant &         \|A^{-1}\|_\infty\left(\|u_{s,1}\|_{L^1(K)} + \|u_{s,2}\|_{L^1(K)}\right),
\end{IEEEeqnarray*}
and
\begin{IEEEeqnarray*}{rCl}
  \|\tilde{u}_{s,2}\|_{L^1(\tilde{K})} & \leqslant & \|A^{-1}\|_\infty\left(\|u_{s,1}\|_{L^1(K)} + \|u_{s,2}\|_{L^1(K)}\right).
\end{IEEEeqnarray*}
Derivatives. If $k,l = 1,2$: 
\begin{IEEEeqnarray*}{rCl}
  \|\partial_{\tilde{x}_l} \tilde{u}_{s,k}\|_{L^1(\tilde{K})} 
  & = & \frac{1}{|\det(A)|} \int\limits_{K} \left|\det(A)\,A^{-1}_{\text{row}_k}\,
  D\bu_s(\bx)\,A_{\text{col}_l}\right|\,d\bx\\
  %& \leqslant & \|A^{-1}_{\text{row}_k}\|_\infty\|A_{\text{col}_l}\|_\infty\sum_{i,j = 1,2} \|\partial_{x_j}u_{s,i}\|_{L^{1}(K)}\\
  & \leqslant & \|A^{-1}\|_\infty\|\partial_{\xi_j}\bu_{s}\|_{L^{1}(K)},
\end{IEEEeqnarray*}
and similarly, if $k=1,2$:
\begin{IEEEeqnarray*}{rCl}
  \|\partial_{\tilde{x}_3} \tilde{u}_{s,k}\|_{L^1(\tilde{K})} & \leqslant &
  \|A^{-1}\|_\infty\sum_{i = 1,2} \|\partial_{\xi_3}u_{s,i}\|_{L^{1}(K)}.
\end{IEEEeqnarray*}
Divergence: because of the blocks of the matrix $A$ we have
\begin{IEEEeqnarray*}{rCl}
  (u_1(\bx), u_2(\bx), 0)^t & = &
\frac{1}{\det A}\,A\, (\tilde{u}_1(\tilde{\bx}), \tilde{u}_2(\tilde{\bx}), 0)^t\text{,}\\
{\text{div}}({u}_1,{u}_2,0)&=&\frac{1}{\det A} \tilde{\text{div}}(\tilde{u}_1,\tilde{u}_2,0).
\end{IEEEeqnarray*}
Then, changing variables,
\begin{IEEEeqnarray*}{rCl}
  \|\tilde{\text{div}}(\tilde{u}_{s,1},\tilde{u}_{s,2},0)\|_{L^{1}(\tilde{K})} &=&
  \|\text{div}(u_{s,1},u_{s,2},0)\|_{L^{1}(K)}.
\end{IEEEeqnarray*}
Joining everything, by~(\ref{matrix1}) and~(\ref{matrix2}), and $\|A\|_\infty\leqslant C$:
\begin{IEEEeqnarray*}{rCl}
  \| (\boldsymbol{r} \bu_s)_1 \|_{L^{1}(K)} & \lesssim & 
  ...
  & & +\;h_1 \|\text{div}(u_{s,1}, u_{s,2}, 0)\|_{L^{1}(K)}.
\end{IEEEeqnarray*}
...
\begin{IEEEeqnarray*}{rCl}
  h_1\|\partial_{x_j}u_{s,i}\|_{L^1(K)} &\leqslant&
...
So we have
\begin{IEEEeqnarray*}{rCl}
  h_1\|\partial_{x_j}u_{s,i}\|_{L^1(K)} & \leqslant & C\,h_1^{1-\delta} |K|^{1/2}
...
\begin{IEEEeqnarray*}{rCl}
  h_3\,\| \partial_{x_3}u_{s,i} \|_{L^1(K)} &=& h_3\,\| \partial_{x_i}u_{s,3} \|_{L^1(K)}\\
  &\leqslant& h_3\,|K|^{1/2}\,\| \partial_{x_i}u_{s,3} \|_{L^2(E)}\\
...
\begin{IEEEeqnarray*}{rCl}
  ...
  h_1 \|\partial_{x_1} u_{s,1}\|_{L^{1}(K)} + h_1\|\partial_{x_2} u_{s,2}\|_{L^{1}(K)}
\end{IEEEeqnarray*}
...
\begin{IEEEeqnarray*}{rCl}
          \| (\boldsymbol{r}\bu_s)_3 \|_{L^{1}(K)}
  & =     &
  \| (\tilde{\boldsymbol{r}}\tilde{\bu}_s)_3 \|_{L^{1}(\tilde{K})}\\
...
\begin{IEEEeqnarray*}{rCl}
  h_3\,\|\partial_{x_3}u_{s,3}\|_{L^1(K)} &\leqslant& C\,|K|^{1/2}\,\textit{h}\,\|f\|_{L^2{(\Lambda_\ell)}}.
\end{IEEEeqnarray*}
At last, having the bound for each of the three components, by~(\ref{normaL2L1}), {\color{BrickRed}(redactar mejor)}
\begin{IEEEeqnarray*}{rCl}
  \|\boldsymbol{r} \bu_s\|_{L^{2}(K)} &\leqslant& C\,\textit{h}\,\|f\|_{L^2(\Lambda_\ell)}.
\end{IEEEeqnarray*}
...
\end{IEEEeqnarray*}
Parte con $d(E,e)>0$. Recordar c\'omo son los nuevos $h_1,h_2,h_3$.
%% (y,z,x)
\begin{center}
...
\end{IEEEeqnarray*}
Parte con ${\color{red} d(E,e)=0}$.
\begin{IEEEeqnarray}{rCl}
  \label{distancia_cero_arista}\sum_{d(E,e) = 0}
