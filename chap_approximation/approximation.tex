...
\|\dvg(\textbf{u} - \pi\textbf{u})\|^2_{0,\Omega} & = &
  ...
  \text{proy. ortog. por elem.}&=&\|\dvg\,\textbf{u} - \boldsymbol{p}_{\tau_h}\dvg\,\textbf{u}\|^2_{0,\Omega}\\
...
\begin{IEEEeqnarray*}{rCl}
  \|\boldsymbol{\sigma}_r - \boldsymbol{r}\boldsymbol{\sigma}_r\|_{L^2(\Omega)}^2 & = &
  \sum_{K\in\mathcal{T}_{\boldsymbol{h}}}
  \|\boldsymbol{\sigma}_r - \boldsymbol{r}\boldsymbol{\sigma}_r\|_{L^2(K)}^2\\
  &=&\sum_{K\in\mathcal{T}_{\boldsymbol{h}}}\left( \sum_{|\alpha| = 1} 
  \boldsymbol{h}^\alpha \|D^\alpha\boldsymbol{\sigma}_r\|_{L^2(K)} + 
  h_K\|\text{div}\boldsymbol{\sigma}_r\|_{L^2(K)}
  \right)^2\\
  &\leqslant&\sum_{K\in\mathcal{T}_{\boldsymbol{h}}}\left[\sum_{|\alpha| = 1}
...
mesh, so that we get
\begin{IEEEeqnarray*}{rCl}
  &\leqslant&\sum_{K\in\mathcal{T}_{\boldsymbol{h}}}4\,\textit{h}^{2}
  \left[\sum_{|\alpha| = 1}\|D^\alpha\boldsymbol{\sigma}_r\|_{L^2(K)}^2 + 
  \|\text{div}\boldsymbol{\sigma}_r\|_{L^2(K)}^2\right]\\
  &=&4\,\textit{h}^{2}
...
Then, finally
\begin{IEEEeqnarray*}{rCl}
  \|\boldsymbol{\sigma}_r - \boldsymbol{r}\boldsymbol{\sigma}_r\|_{L^2(\Omega)}&\leqslant&
  C\,\textit{h}
  \left(\sum_{|\alpha| = 1}\|D^\alpha\boldsymbol{\sigma}_r\|_{L^2(\Omega)} + 
  \|\text{div}\boldsymbol{\sigma}_r\|_{L^2(\Omega)}\right).
\end{IEEEeqnarray*}
{\color{red} C\'omo pasamos a la norma de $f$?}
\subsection{bound singular part prismatic macroelement} % (fold)
...
\begin{IEEEeqnarray*}{rCl}
  ...
  \sum_{\text{d}(K,e) = 0} \left(\| \boldsymbol{\sigma}_s\|_{L^2(K)} + 
  \|\boldsymbol{r}\boldsymbol{\sigma} _s\|_{L^2(K)}\right)^2\\
  & &\:+\sum_{\text{d}(K,e) > 0} \left( \sum_{|\alpha| = 1} 
  \boldsymbol{h}^\alpha \|D^\alpha\boldsymbol{\sigma}_s\|_{L^2(K)} + 
  h_K\|\text{div}\boldsymbol{\sigma}_s\|_{L^2(K)}
  \right)^2.
\end{IEEEeqnarray*}
...
\subparagraph{Bound for $\|\partial_{x_1}\boldsymbol{\sigma}_s\|_{L^2(K)}$.} % (fold)
...
\subparagraph{Bound for $\|\partial_{x_3}\boldsymbol{\sigma}_s\|_{L^2(K)}$.} % (fold)
...
\subparagraph{Bound for $\| \boldsymbol{\sigma}_s\|_{L^2(K)}$.} % (fold)
\label{subp:bound_for_sigma_i}
\begin{IEEEeqnarray*}{rCl}
  \|\boldsymbol{\sigma}_s\|_{L^2(K)}& = & \sum_{i=1,2,3}\|{\sigma_{s,i}}\|_{L^2(K)}\\
  &=&\sum_{i=1,2,3}\|r(\textbf{x})^{1-\delta} r(\textbf{x})^{\delta-1} {\sigma_{s,i}}\|_{L^2(K)}\\
...
Parte singular:
\begin{IEEEeqnarray*}{rCl}
  \|\textbf{u}_s - \pi\textbf{u}_s\|^2_{\scriptscriptstyle L^2(\Omega)^3}
  &=&\sum_{d(K,e) = 0}
  \|\textbf{u}_s - \pi\textbf{u}_s\|^2_{\scriptscriptstyle L^2(K)^3}+
  \sum_{d(K,e) > 0}
  \|\textbf{u}_s - \pi\textbf{u}_s\|^2_{\scriptscriptstyle L^2(K)^3}\\[7pt]
\end{IEEEeqnarray*}
...
$K$ es una pir\'amide
\begin{IEEEeqnarray*}{rCl}
  \|\textbf{u}_s - \pi\textbf{u}_s\|^2_{\ss L^2(K)}&\leqslant&
