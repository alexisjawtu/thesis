...
First we'd like to remark that, for a given macro--element in $\pazocal{T}_{\textit{h}_0}$ the case when there is no weight with respect to the distance $R(\bx)$
to the vertex is also available by simply putting 
$R\equiv 1$ in the computations we show. The case when there is no weight with respect to the angular distance
$\theta(\bx)$ is also available by simply putting 
$\theta\equiv 1.$ Additionally, if $\be$ is not singular, the condition for $\delta$ in~(\ref{label_grading}) 
turns $\delta > -\infty$ and if $\bv$ is not singular, the condition for it
turns $\beta > -\infty.$
...
let $\boldsymbol{h}=(h_{1},h_{2},h_{3})'=(h_{E,1},h_{E,2},h_{E,3})'$. By Theorem~\ref{aux_label46}
\begin{IEEEeqnarray*}{rCl}
  \|\bu_r - \br_0\bu_r\|_{L^2(\Omega)}^2 & = &
  \sum_{E\in\Th}
  \|\bu_r - \br_0\bu_r\|_{L^2(E)}^2\\
  &\leqslant&C\,\sum_{E\in\Th}\left( \sum_{|\alpha| = 1} 
...
\subsection{Bound for the Singular Part in a Prismatic Macro--Element with
a singular Edge} % (fold)
\label{sub:bound_singular_part_prismatic_macroelement}
The bound for the error of the interpolation of the singular part will be 
...
\begin{IEEEeqnarray*}{rCl}
  ...
  (\mbox{$\mu \leqslant 1$})&\leqslant& C\,\textit{h}^2\|\bu_{s}\|^2_{\pazocal{V}_\delta^{1,2}(E)}.
\end{IEEEeqnarray*}
...
\begin{IEEEeqnarray*}{rCl}
  \|u_{s,3}\|_{L^1(E)} &\leqslant& \|r\|_{L^2(E)} \,\|r^{-1}\,u_{s,3}\|_{L^2(E)}\\
              &\leqslant&C\,h_1\,|E|^{\frac{1}{2}}\,\|u_{s,3}\|_{V_0^{1,2}(E)}\\
              &\leqslant&C\,\textit{h}\,|E|^{\frac{1}{2}}\,\|u_{s,3}\|_{V_0^{1,2}(E)}. 
\end{IEEEeqnarray*}
...
\end{IEEEeqnarray*}
At last, having the bound for each of the three components, by~(\ref{normaL2L1}),    %% TODO {\color{BrickRed}(redactar mejor)}
\begin{IEEEeqnarray*}{rCl}
  \|\br \bu_s\|_{L^{2}(E)} &\leqslant& C\,\textit{h}\,\|\bu_s\|_{\pazocal{V}_{\delta}(E)}.
\end{IEEEeqnarray*}
\end{enumerate}
...
with Singular Edge and Vertex} % (fold)
...
elements touching the singular edge in the prismatic macroelement. Now
