...
\begin{IEEEeqnarray*}{rCl}
  \|(\br_0\bu_s)_1\|_{L^1(T_\ell)} &=& \int_{\tilde{T}}|(M_{T_\ell}\tilde{\br}_0\tilde{\bu}_s)_1|\,d\tilde{\bx}\\[7pt]
    &\leqslant& C \sum_{1\leqslant i\leqslant 3} \left(\|\tilde{u}_{s,i}\|_{L^1(\tilde{T})} + 
      ...
      h_i\|\dvg \tilde{\bu}_s\|_{L^1(\tilde{T})}
    \right) \\[7pt] %%% & = & \ldots \text{paso al tetraedro f\'isico} \ldots \\[7pt] % VER BORRADOR INTERPOLACION PIRAMIDE
    & \leqslant & C
    \left({\color{orange} \|\bu_{s}\|_{L^1(T_\ell)^3}} + 
    \sum_{1\leqslant j \leqslant 3} {\color{teal} h_j\|\partial_{\xi_j}\bu_{s}\|_{L^1(T_\ell)}} + 
    {\color{purple} (h_1+h_2+h_3) \|\dvg \bu_s\|_{L^1(T_\ell)}}\right)
\end{IEEEeqnarray*}
\noindent{\color{Orange}\#\#\#\#\#\#\# where $C$ depends only on the max angle?}. 
PESOS
\begin{IEEEeqnarray*}{rCl}
  {\color{Orange} \|\bu_{s}\|_{L^1(E)^3}} & = &
