...
we will use Proposition~\ref{propErrorInterpolacionPiramidesTetraedros} as the
local interpolation estimate.
Nevertheless, as we will show later, 
we proved 
anisotropic pyramidal finite element stability estimates
in Theorems~\ref{aux_label53}  and~\ref{aux_label54}, and with them
we obtained the results in Theorems~\ref{auxlabel402} and~\ref{auxlabel211}
for the interpolation operators determined by the 
pyramidal finite elements in Definitions~\ref{aux_label50} and~\ref{aux_label71}.
So, we could use those anisotropic estimates from Chapter~\ref{auxlabel202},
and try to make the estimates in terms of weighted norms 
exactly as were made for the anisotropic prisms in the method we present here.
These observations are made to point out that we may as well 
propose a full Finite Elements Method as an alternative.\\[6pt]
...
solution
of Problem~\ref{weakMixedContinuous}. Let  $\br_0$ be the operator~(\ref{global_interpolator})
corresponding to the mesh constructed in Subsection~\ref{meshes}, then
...
\label{auxlabel360}
\end{IEEEeqnarray}
\paragraph{Terms in~\eqref{auxlabel360} such that $d(E,\be)>0$. }
%% (y,z,x)
\begin{figure}[!h]
  \centering
...
\end{enumerate}
\paragraph{Terms in~\eqref{auxlabel360} such that $d(E,\be_\ell)=0$.}
...
\begin{IEEEeqnarray}{rCl} %
  ...
  \,+\sum_{
  \substack{{\scriptstyle\text{prisms\,}P}\\
