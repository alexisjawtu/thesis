...
degrees of freedom and the very definition of the operator.
...
\end{proof}
\begin{lemma} If $\hat\bu\in P_{\hat E}$ is such that $\hat{u}_1 = 0$ and $\hat{u}_2 = 0$, then $(\hat{\boldsymbol{r}}_k\hat{\bu})_1
...
\begin{proof} We use again the expression~(\ref{exprPrt}) for $\hat{\boldsymbol{r}}_k\hat{\bu} =
  ...
  $\hat{q}\in\mathcal{P}_{k-1}(\hat f_1)\otimes\mathcal P_{k-1}(\hat z)$ and use
  Green's Theorem. By the surface degrees of freedom~\ref{momentos2hdiv}
  ...
  \begin{IEEEeqnarray*}{rClCr}
    ...
    - \int\limits_{\hat{E}} (\hat{p}_1, \hat{p}_2, 0)^t\cdot\nabla \hat{q}\,d\hat{\textbf{x}}
    + \int\limits_{\partial\hat{E}} (\hat{p}_1, \hat{p}_2, 0)^t\cdot\boldsymbol{\hat\nu}\,\hat{q}\,d\gamma
    & = & 0.
  \end{IEEEeqnarray*}
  And since $\text{div}(\hat{p}_1, \hat{p}_2, 0)^t$ also belongs to $\pp{k-1}{k-1}$, we just proved it vanishes on all $\hat{E}$.
  \noindent{\color{blue}\#\#\#\#\#\#\# HERE!} 
    ...
    \begin{IEEEeqnarray*}{rCl}
        ...
        p_2 & = & y\,\psi.
    \end{IEEEeqnarray*}
    But finally, by the degrees of freedom~(\ref{momentos3hdiv}), $p_1$ and $p_2$ vanish entirely on $\hat{E}$. 
\end{proof}
