...
\begin{IEEEeqnarray*}{rCl}
  \int\limits_{\hat{E}} ((\rku)_3)^2\,d\hat{\bx} & = & 0
\end{IEEEeqnarray*}
...
let first $\hat{f} = \hat{f}_5$. Pick $q\in Q_{k-1,k-1,k-1}$.
\begin{IEEEeqnarray*}{rCl}
    \int\limits_{\hat{f}} \hat{\br}_k\hat{\bv}\cdot\boldsymbol{\nu}\,\hat{q}\,d\hat{\gamma} &=&
    \int\limits_{\hat{f}} (\hat{v}_1 + \hat{v}_2)\,\hat{q}\,d\hat{\gamma}.
\end{IEEEeqnarray*}
Observe that the restriction of $\hat{q}$ to $\hat{f}$ belongs to
$P_{k-1}(\hat{x}_1)\otimes P_{k-1}(\hat{x}_3)$. Now
\begin{IEEEeqnarray*}{rCl}
  \int\limits_{\hat{E}} \mbox{div} (\hat{u}_1,\hat{u}_2,0)^t\,\hat{q}\,d\hat{\bx}
  & = &  \int\limits_{\hat{E}} \mbox{div}\,\hat{\bv} \,\hat{q}\,d\hat{\bx}\\
  & = & -\int\limits_{\hat{E}} (\hat{u}_1, \hat{v}_2, 0)^t\cdot\nabla \hat{q}\,d\hat{\bx} + 
         \int\limits_{\partial_{\hat{E}}} (\hat{u}_1, \hat{v}_2, 0)^t\cdot\hat{\boldsymbol{\nu}}\,
         \hat{q}\,d\hat{\gamma}\\
  & = & -\int\limits_{\hat{E}} \hat{u}_1 \frac{\partial\hat{q}}{\partial\hat{x}_1}\,d\hat{\bx}
        -\int\limits_{\hat{f}_1}\hat{u}_1\,\hat{q}\,d\hat{\gamma}
        +\int\limits_{\hat{f}} (\hat{v}_1 + \hat{v}_2)\,\hat{q}\,d\hat{\gamma},
\end{IEEEeqnarray*}
Rearranging,
\begin{IEEEeqnarray*}{rCl}
    \int\limits_{\hat{f}} (\hat{v}_1 + \hat{v}_2)\,q\,d\hat{\gamma} & = & \int\limits_{\hat{E}} \mbox{div} (\hat{u}_1,\hat{u}_2,0)
    \,q\,d\textbf{x} + \int\limits_{\hat{E}} \hat{u}_1\partial_xq \,d\hat{\bx} +
    \int\limits_{f_1}\hat{u}_1\,q\,d\hat{\gamma}.
\end{IEEEeqnarray*}
For the volume degrees of freedom~(\ref{momentos3hdiv}) take $q_2 \in P_{k-2}(x,y) \otimes P_{k-1}(\hat{x}_3)$. Since
\begin{IEEEeqnarray*}{rCl}
    \hat{v}_2(x,y,z) & = &\int\limits_0^{y} \partial_y \hat{u}_2(x,t,z) \,dt,
\end{IEEEeqnarray*}
we may do
\begin{IEEEeqnarray*}{rCl}
    \int\limits_{\hat{E}} \hat{v}_2\,q_2  & = &\int\limits_0^1\int\limits_0^1\int\limits_0^{1-x}
    \int\limits_0^{y}
      \partial_y \hat{u}_2(x,t,z) \,dt \,q_2 (x,y,z)\,dy\,dx\,dz\\
        & = &\int\limits_0^1\int\limits_0^1\int\limits_0^{1-x}\int\limits_0^{y}
                \partial_y \hat{u}_2(x,t,z) \,q_2 (x,y,z)\,dt\,dy\,dx\,dz\\
        & = &\int\limits_0^1\int\limits_0^1\int\limits_0^{1-x}\int\limits_t^{1-x}
                \partial_y \hat{u}_2(x,t,z) \,q_2 (x,y,z)\,dy\,dt\,dx\,dz\\
        & = &\int\limits_0^1\int\limits_0^1\int\limits_0^{1-x}\partial_y \hat{u}_2(x,t,z)
                \int\limits_t^{1-x}\,q_2 (x,y,z)\,dy\,dt\,dx\,dz\\
        & = &\int\limits_0^1\int\limits_0^1\int\limits_0^{1-x}\partial_y \hat{u}_2(x,t,z)\,\phi (x,t,z)\,dt\,dx\,dz\\
& = &\int\limits_{\hat{E}} (\partial_x\hat{u}_1 + \partial_y \hat{u}_2)\,\phi\,d\hat{\bx}
    - \int\limits_{\hat{E}} \partial_x\hat{u}_1\,\phi\,d\hat{\bx}\\
& = &\int\limits_{\hat{E}} \mbox{div} (\hat{u}_1,\hat{u}_2,0)^t\,\hat{\phi}\,d\hat{\bx}
    - \int\limits_{\hat{E}} \partial_x\hat{u}_1\,\hat{\phi}\,d\hat{\bx}
\end{IEEEeqnarray*}
(for some $\hat{\phi} \in \mathcal{P}_{k-1}(\hat{f}_3)\otimes\mathcal{P}_{k-1}(\hat{x}_3)$),
as we needed. 
\end{proof}
