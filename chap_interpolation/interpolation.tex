...
There is $C > 0$, independent of $h_1$, $h_2$ and $h_3$, s.t. for all $p \geqslant 1$ and 
  ...
  \begin{IEEEeqnarray*}{rCl}
    ...
    + h_{\tilde{E}}\left\|\mathit{\dv}(\tilde{u}_1, \tilde{u}_2, 0) \,\right\|_{L^p(\tilde{E})}\right).
  \end{IEEEeqnarray*}
\end{theorem}
\begin{proof}
...
\begin{IEEEeqnarray*}{rCl}
  ...
  &\lesssim& \|u_1\|_{\scriptscriptstyle{W^{1,p}(\hat{E})}} +
    \|\dv \bu\|_{\scriptscriptstyle{L^p}(\hat{E})} + \left\|\dfrac{{\scriptstyle\partial} u_3}{{\scriptstyle\partial} x_1}\right\|_{\scriptscriptstyle{L^p}(\hat{E})}
      + \left\|\dfrac{{\scriptstyle\partial} u_3}{{\scriptstyle\partial} x_2}
        \right\|_{\scriptscriptstyle{L^p}(\hat{E})} + \left\|\dfrac{{\scriptstyle\partial} u_2}{{\scriptstyle\partial} x_2} 
        \right\|_{\scriptscriptstyle{L^p}(\hat{E})}.\\[12pt]
  \|(\rku)_2\|_{\scriptscriptstyle{L^\infty(\hat{E})}}
  &\lesssim& \|u_2\|_{\scriptscriptstyle{W^{1,p}(\hat{E})}} +
    \|\dv \bu\|_{\scriptscriptstyle{L^p}(\hat{E})} + 
    \left\|\dfrac{{\scriptstyle\partial} u_3}{{\scriptstyle\partial} x_1}
      \right\|_{\scriptscriptstyle{L^p}(\hat{E})} + 
    \left\|\dfrac{{\scriptstyle\partial} u_3}{{\scriptstyle\partial} x_2}
      \right\|_{\scriptscriptstyle{L^p}(\hat{E})} + 
    \left\|\dfrac{{\scriptstyle\partial} u_1}{{\scriptstyle\partial} x_1}
        \right\|_{\scriptscriptstyle{L^p}(\hat{E})}.\\[12pt]
...
\begin{proof}
We will use the notation of Table~(\ref{shape_edge_table}). The variables 
in the local coordinate system of $\hat E$ are $x,y$ and $z$.
...
\begin{IEEEeqnarray*}{rCl}
  ...
  (\alpha_1(\hat\bu)+\alpha_4(\hat\bu))\,\frac{2y-yz}{1-z}\\[4pt]
    & = & -2{\iint_{\hat{f}_1} \hat{\bu} \cdot \hat\bn_2\,d\gamma} + 
...
Then
\begin{IEEEeqnarray*}{rCl}
  (\rku)_1\xyz & = & {\int_{\hat{E}} \dv\hat{\bu}\,d\hat{\boldsymbol{x}}}\,x
                      + (\alpha_3(\hat\bu)-\alpha_4(\hat\bu))\frac{x}{1-z}.
\end{IEEEeqnarray*}
