...
\end{theorem}
\begin{proof} The proof is based on the last three Lemmas. Let
...
\subsection{Stability $\tilde{K}$. R-T.} % (fold)
\label{sub:stability_ktilde_r_t}
\begin{theorem} \label{thmStabilityKtildeRT}
There is $C > 0$, independent of $h_1$, $h_2$ and $h_3$, s.t. for all $p \geqslant 1$ and 
  $\tilde{\mathit{\textbf{u}}}\in W^{1,p}(\tilde{K})$
  \begin{IEEEeqnarray*}{rCl}
    \left\| \tilde{\boldsymbol{r}}_k\,\tilde{\mathit{\textbf{u}}}\right\|_{L^p(\tilde{K})}
    & \leqslant & C \left( \left\| \tilde{\mathit{\textbf{u}}} \right\|_{L^p(\tilde{K})}
    + \sum_{i=1}^3 h_i \left\| \partial_{\tilde{x}_i}\tilde{\mathit{\textbf{u}}} \right\|_{L^p(\tilde{K})}
    + h_{\tilde{K}}\left\|\mathit{\Div}(\tilde{u}_1, \tilde{u}_2, 0) \,\right\|_{L^p(\tilde{K})}\right).
  \end{IEEEeqnarray*}
\end{theorem}
\begin{proof} 
Piola transform $+$ change of variables $+$ stability inequality~(\ref{teoremaDiv_1}) 
$+$ H\"older's inequality.
Take $p\geqslant 1$.
\begin{IEEEeqnarray*}{rCl}
  \|(\tilde{\boldsymbol{r}}_k\tilde{\boldsymbol{u}})_1\|_{L^{\infty}(\tilde{K})} & = &
  \frac{1}{h_2h_3}
  \|(\hat{\boldsymbol{r}}_k\hat{\boldsymbol{u}})_1\|_{L^{\infty}(\hat{K})}\\[7pt]
  &\leqslant&\frac{1}{h_2h_3}\left(\|\hat{\boldsymbol{u}}_1\|_{W^{1,1}(\hat{K})}
  +\|\dvg(\hat{u}_1,\hat{u}_2,0)\|_{L^{1}(\hat{K})}\right)\\[7pt]
  {\color{red}revisar}&=& 
  \frac{1}{h_2h_3}\left(
  \int\limits_{\hat{K}}|\hat{u}_1|\,d\hat{\textbf{x}}
  +\sum_{i=1}^3\int\limits_{\hat{K}}|\partial_{\hat{x}_i}\hat{u}_1|\,d\hat{\textbf{x}}
  +\int\limits_{\hat{K}}|\partial_{\hat{x}_1}\hat{u}_1 + \partial_{\hat{x}_2}\hat{u}_2|
  \,d\hat{\textbf{x}}\right)\\[7pt]
  &=&\frac{1}{\det F}\left(
  \int\limits_{\tilde{K}}|\tilde{u}_1|\,d\tilde{\textbf{x}}
  +\sum_{i=1}^3h_i\int\limits_{\tilde{K}}|\partial_{\tilde{x}_i}\tilde{u}_1|\,d\tilde{\textbf{x}}
  +h_1\int\limits_{\tilde{K}}|\partial_{\tilde{x}_1}\tilde{u}_1 + \partial_{\tilde{x}_2}\tilde{u}_2|
  \,d\tilde{\textbf{x}}\right)\\[7pt]
  &=&\frac{1}{2|\tilde{K}|}\left(
  \|\tilde{u}_1\|_{L^1(\tilde{K})}
  +\sum_{i=1}^3h_i\|\partial_{\tilde{x}_i}\tilde{u}_1\|_{L^1(\tilde{K})}
  +h_1\|\dvg(\tilde{u}_1,\tilde{u}_2,0)\|_{L^1(\tilde{K})}\right)\\[7pt]
  &\leqslant&\frac{1}{2|\tilde{K}|^{\frac{1}{p}}}\left(
  \|\tilde{u}_1\|_{L^p(\tilde{K})}
  +\sum_{i=1}^3h_i\|\partial_{\tilde{x}_i}\tilde{u}_1\|_{L^p(\tilde{K})}
  +h_1\|\dvg(\tilde{u}_1,\tilde{u}_2,0)\|_{L^p(\tilde{K})}\right)
\end{IEEEeqnarray*}
Now
\begin{IEEEeqnarray*}{rCl}
  \|(\tilde{\boldsymbol{r}}_k\tilde{\boldsymbol{u}})_1\|_{L^{p}(\tilde{K})}
  &\leqslant&
  |\tilde{K}|^{1/p}\|(\tilde{\boldsymbol{r}}_k\tilde{\boldsymbol{u}})_1\|_{L^{\infty}(\tilde{K})}\\
  &\lesssim&\|\tilde{u}_1\|_{L^p(\tilde{K})}
  +\sum_{i=1}^3h_i\|\partial_{\tilde{x}_i}\tilde{u}_1\|_{L^p(\tilde{K})}
  +h_1\|\dvg(\tilde{u}_1,\tilde{u}_2,0)\|_{L^p(\tilde{K})},
\end{IEEEeqnarray*}
and again, the same holds for component $2$. For component $3$
stability inequality~(\ref{teoremaDiv_3}) gives us
\begin{IEEEeqnarray*}{rCl}
  \|(\tilde{\boldsymbol{r}}_k\tilde{\boldsymbol{u}})_3\|_{L^{\infty}(\tilde{K})} & = &\frac{1}{h_1h_2}
  \|(\hat{\boldsymbol{r}}_k\hat{\boldsymbol{u}})_3\|_{L^{\infty}(\hat{K})}\\[6pt]
  &\leqslant&\frac{C}{h_1h_2}\,\|\hat{u}_3\|_{W^{1,1}(\hat{K})}\\[6pt]
  &=&\frac{C}{|\tilde{K}|}\,\left[\|\tilde{u}_3\|_{L^1(\tilde{K})} +
    \sum_{i=1,2,3} h_i\,\|\partial_{\tilde{x}_i}\tilde{u}_3\|_{L^1(\tilde{K})}\right]\\[6pt]
  &\leqslant& \frac{C}{|\tilde{K}|^{\frac{1}{p}}}\,\left[\|\tilde{u}_3\|_{L^p(\tilde{K})} +
    \sum_{i=1,2,3} h_i\,\|\partial_{\tilde{x}_i}\tilde{u}_3\|_{L^p(\tilde{K})}\right],
\end{IEEEeqnarray*}
for any real $p \geqslant 1$. So, immediately,
\begin{IEEEeqnarray*}{rCl}
  \|(\tilde{\boldsymbol{r}}\tilde{\boldsymbol{u}})_3\|_{L^{p}(\tilde{K})}
  &\leqslant& C\,\left(
  \|\tilde{u}_3\|_{L^p(\tilde{K})} +
    \sum_{i=1,2,3} h_i\,\|\partial_{\tilde{x}_i}\tilde{u}_3\|_{L^p(\tilde{K})}
  \right)
\end{IEEEeqnarray*}
and the sum of the three estimates yields the theorem.
\end{proof}

% subsection stability_ktilde_r_t (end)
