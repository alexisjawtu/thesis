...
\end{IEEEeqnarray}
\noindent{\color{blue}\#\#\#\#\#\#\# CONTINUE HERE hacer bien partes}
With the same technique for~(\ref{momentos5hcurl}) on $\hat{f}_5$, if  given
$\bq = (0,q_3,q_1) \in \{ 0 \} \times Q_{k-2,k-1} \times Q_{k-1,k-2}$ we 
define $\eta(\hat{x}_1,\hat{x}_3) = -\int_{x}^{1} q_1(t,\hat{x}_3)\,dt$, 
we prove
\begin{IEEEeqnarray}{rCl}\label{momentosWcaras3}
  \nonumber\int\limits_{\hat{f}_5} \hat{\bv} \times \boldsymbol{\nu} \cdot \hat{\bq}\,d\hat{\gamma}
    & = & \int\limits_{\hat{f}_5} \hat{u}_1\,\hat{q}_1\,d\hat{\gamma} +
    \int\limits_{\hat{f}_5} (\curl\hat{\bu})_3\,\eta\,d\hat{\gamma}\\
    &   & - \int\limits_{\hat{f}_5} \hat{u}_1\,\partial_{\hat{x}_2}\eta\,d\hat{\gamma}  +
    \int\limits_{\hat{\be}_4} \hat{u}_1\,\eta\,d\hat{s}.
\end{IEEEeqnarray}
