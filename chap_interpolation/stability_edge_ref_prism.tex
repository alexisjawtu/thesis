...
\begin{lemma}\label{lema_PIu3_k_cualquiera} Let $\hat\bu\in$ {\color{red}dominio.} Then 
$(\hat{\boldsymbol{w}}_k\hat{\bu})_3$ is linearly and univocally 
...
For inequality~(\ref{teorema_3}) 

Definimos, dada $\hat{\bu} \in W^{1,p}(\hat{K})^3$,
\begin{IEEEeqnarray*}{rCl}
	\hat{\bv} & = & (0,0, \hat{u}_3). \mbox{{\color{red} CONTINUE HERE}}
\end{IEEEeqnarray*}
Como dec\'{\i}amos, observemos primero que gracias a los lemas~(\ref{lemma_PIu2_k_in_N}) 
y~(\ref{pi00u3}) es
\begin{IEEEeqnarray*}{rCl}
...
\begin{IEEEeqnarray*}{rCl}
  (\hat{\boldsymbol{w}}_k\hat{\bu})_3 
    & = & \sum_{e,p} \varphi_{e,p}(\hat{\bu})\,(\hat{\bv}_{e,p})_3 +
          \sum_{f,q} \varphi_{f,q}(\hat{\bu})\,(\hat{\bv}_{f,q})_3 +
          \sum_{r}   \varphi_{r}  (\hat{\bu})\,(\hat{\bv}_{r})_3
\end{IEEEeqnarray*}
but at the beggining of the proof of Lemma~(\ref{lema_PIu3_k_cualquiera})





if we look at expressions~(\ref{aristas}),~(\ref{caras}) and~(\ref{enK})
we have 

\begin{IEEEeqnarray}{lClc}
	|\int\limits_{e_j} (\pi\textbf{u})_3 q\,ds|
	& \leqslant & \int\limits_{e_j} |\textbf{u}_3 q| \, ds, &\quad q\in P_{k-1}(e_j).
\end{IEEEeqnarray}
\begin{IEEEeqnarray}{lClc}
	|\int\limits_{f} (\pi\textbf{u})_3 q\,d\gamma|
	& \leqslant & \int\limits_{f} |\textbf{u}_3 q|\,d\gamma, 
	&\quad q\in Q_{k-2,k-1}(f_{ijkl}).
\end{IEEEeqnarray}
\begin{IEEEeqnarray}{lClc}
	|\int\limits_{\hat{K}} (\pi\textbf{u})_3 q\,d\textbf{x}|
	& \leqslant & \int\limits_{\hat{K}} |\textbf{u}_3 q|\,d\textbf{x}, 
	&\quad q\in P_{k-3}(f_z) \otimes P_{k-1}(e_{xy}).
\end{IEEEeqnarray}	
...
determinaci\'on de $\hat{\pi}(\hat{\bu})_3$ por parte de $\hat{u}_3$.
