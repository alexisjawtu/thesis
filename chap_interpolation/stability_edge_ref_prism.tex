...
\begin{IEEEeqnarray*}{rCCCl}
	\int\limits_{\hat{E}} \hat{x}_3\,(\hat{x}_3-1)\,\hat\psi^2\,d\hat{\boldsymbol{x}}
  & = &\int\limits_{\hat{E}} (\curl\wku)_3\,\hat\psi\,d\hat{\boldsymbol{x}}&&\\
  & = &\int\limits_{\hat{E}} (\br_k\curl\hat{\bu})_3\,\hat{\psi}\,d\hat{\boldsymbol{x}}&&\\
  & = &\int\limits_{\hat{E}} (\curl\hat{\bu})_3\,\hat{\psi}\,d\hat{\boldsymbol{x}} & = & 0\\
\end{IEEEeqnarray*}
and it follows that
\begin{IEEEeqnarray}{rCl}
...
\label{teorema_1} \norm{(\wku)_1}_{L^{\infty}(\hat{E})} & 
	...
	\|(\curl\hat{\bu})_3\|_{{\color{red} W^{1,1}(\hat{E})}} \\	
...
we get
\begin{IEEEeqnarray*}{rCl}
	\tfrac{1}{\sqrt{2}} \int\limits_{\hat{\be}_9} (\hat{v}_1 - \hat{v}_2)\,\phi\,ds & = &\\
...
\begin{IEEEeqnarray}{rCl}\label{momentosWcaras}
 	\nonumber\int\limits_{\hat{f}_3} \hat{\bv} \times \boldsymbol{\nu} \cdot \bq\,d\hat{\gamma}
...
\begin{IEEEeqnarray}{rCl}\label{momentosWcaras2}
 	\nonumber\int\limits_{\hat{f}_4} \hat{\bv} \times \boldsymbol{\nu} \cdot \bp\,d\hat{\gamma}
...
\begin{IEEEeqnarray*}{rCCCl}
  ...
  \sum_{\be,p} |\varphi_{\be,p}(\hat{\bu})|\,\|(\hat{\bv}_{\be,p})_1\|_{L^\infty(\hat{E})} +
  \sum_{f,\bq} |\varphi_{f,\bq}(\hat{\bu})|\,\|(\hat{\bv}_{f,\bq})_1\|_{L^\infty(\hat{E})} +
  \sum_{\boldsymbol{r}} |\varphi_{\boldsymbol{r}}(\hat{\bu})|\,
  \|(\hat{\bv}_{\boldsymbol{r}})_1\|_{L^\infty(\hat{E})}
  }
  \\
  %\left\|\sum F_{i}(\hat{\textbf{w}}, \textbf{q}_i)\,\hat{\textbf{v}}_i\right\|_{L^\infty(\hat{E})}\\
	%& \leqslant & C \sum \left|F_{i}(\hat{\textbf{w}}, \textbf{q}_i)\right|\,
%		\left\|\hat{\textbf{v}}_i\right\|_{L^\infty(\hat{E})}\\
	& \lesssim & \|\hat{u}_1\|_{W^{1,p}(\hat{E})} & + &
...
\noindent{\color{blue}\#\#\#\#\#\#\# CONTINUE HERE: acomodar la frase para la 
demostr. de esta desigualdad. Buscar un buen lugar para poner la menci'on a monk 
porque se usa en las tres cotas.} 
\begin{IEEEeqnarray*}{rCl}
  (\hat{\boldsymbol{w}}_k\hat{\bv})_3 & = &
  \sum_{j=3,6,7\,p\,\in\,{\color{red}\mathcal{B}_{\hat e_j}}}
  \int\limits_{\hat e_j} \hat{u}_3 p\,ds \,(\hat{\bv}_{e_j,p})_3 +
