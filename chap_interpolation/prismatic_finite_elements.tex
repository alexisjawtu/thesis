...
\begin{IEEEeqnarray}{rCl}\label{aux_label30}
  \|\tilde \bu - \tilde \bq\|_{L^p(\tilde E)}& \leqslant &
    C \sum_{i+j+k=m+1} h_1^ih_2^jh_3^k \left\| \frac{\partial^{m+1}\tilde\bu}
...
estas desig de taylor prom yo solo las uso
directo en Lp, por eso ya estaria acomodado
lo de la medida de E a la (1-1/p) etc...
ver despues de '...is finite measured...' y lo uso como ahi sin 
lo de la medida. Despues de esto, terminar el teorema de interpolacion local}
...
\begin{IEEEeqnarray}{rCl}
  ...
  \label{aux_label25}
  \|\frac{\partial}{\partial_{\tilde x_i}}\curl(\tilde\bu-\tilde\bq)_3\|_{L^p(\tilde E)}&\leqslant&
  C\,h^{m-1}
...
\end{IEEEeqnarray*}
\end{remark}
\begin{proof}[Proof of Theorem~\ref{aux_label23}] {\color{blue}\#\#\#\#\#\#\#\# Ariel, por favor mirar si es correcta
...
\begin{IEEEeqnarray}{rCl}\label{aux_label37}
...
\label{aux_label33}
\lesssim\sum_{i+j+k=m+1} h_1^ih_2^jh_3^k \left\| \frac{\partial^{m+1}\tilde\bu}
    {\partial\tilde x_1^i\partial\tilde x_2^j\partial\tilde x_3^k}
    \right\|_{L^p(\tilde E)}
+ h^{m+1} \|\partial^m(\curl\tilde\bu)_3\|_{L^p(\tilde E)}
\end{IEEEeqnarray}
...
with~(\ref{aux_label34}) and~(\ref{aux_label37}) to get
