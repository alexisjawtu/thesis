...
of $W^{1,1}(\hat E)$, a space in which the elements have well defined
...
$H^{1/2+\delta}(\hat{E})^3$, but the latter does not suffice
to obtain our results.
For the whole subsection, $\hat\br_{\hat E}$ will be the $k$--th order face 
interpolation operator on the reference
Prism determined by the finite element of
Definition~\ref{defi_h_div_conforme}.

In~\cite{MR1866274} there are also local anisotropic estimates for prismatic
elements conforming in $H(\Div)$, which are made with a different
technique, and which are only for the least order case.
Here we prove local anisotropic  estimates for arbitrary order
interpolation.

Recall that $\hat u_i$ and $(\rku)_i$ denote the components of
$\hat\bu$ and $\rku$ respectively.
...
Now set $\hat u_3 = 0$, which makes the right hand side of all the 
equations in~(\ref{rku_3_1})
and~(\ref{rku_3_2}) equal to zero. We will prove that the present
square homogeneous linear system has a unique trivial solution, and the 
Lemma will follow.
...
\end{proof}
\begin{lemma}\label{lemma_u1_u2} Take an element
$\hat\bu$ of the form
$\hat\bu\xyz = (0,0, \hat{u}_3\xyz)'$. 
...
freedom~(\ref{momentos2hdiv}) and the volume degrees of freedom~(\ref{momentos3hdiv}) we have
  ...
  proof of Lemma~\ref{auxlabel500} onwards). 
  This means we may
...
\begin{enumerate}
  ...
  \begin{IEEEeqnarray}{rCl}
    \label{aux_label25}
    \left\|\tfrac{\partial\curl(\tilde\bu-\tilde\Qbb_{m,\tilde{E}}\tilde{\bu})_i}{\partial\tilde x_j}
    \right\|_{L^p(\tilde E)}
      &\leqslant& 
...
\input{./chap_interpolation/div_prism_local_error}
%
