...
\begin{theorem} \label{auxlabel211} Let $E$ be any pyramid which is a non degenerate affine image 
  of the reference pyramid $\hat{E}$ with sizes $h_1, h_2$ and $h_3$
  such that $h_3\geqslant Ch_1$ or $h_3\geqslant Ch_2$.
  Let $k\in\mathbb{N}$ and $p>2$ and let $E$ be a prism whose triangular
...
\begin{IEEEeqnarray*}{rCl}\label{aux_label55}
  ...
  \left\{
    \sum_{|{\balpha}|=1}\bh^{\balpha} \|\partial^{\balpha} \bu\|_{L^p(E)} +\right.\\[4pt]
  \yesnumber\label{auxlabel5}
   &&\qquad\left. h_E\sum_{|{\balpha}|=1}\bh^{\balpha}\|\partial^{\balpha} 
    (\curl \bu)_3 \|_{L^p(E)}
  \right\}.\\
  & &\max \{h_{i}\}^2 |\tilde{\bu}|_{2,p,\tilde{E}} \\[5pt]
  & &\,+\, \max \{h_{1}, h_2\} \big( \|\partial_{\tilde{x}_1}\tilde{u}_2\|_{\scriptscriptstyle L^p(\tilde E)}
   + \|\partial_{\tilde{x}_2}\tilde{u}_1\|_{\scriptscriptstyle L^p(\tilde E)}
\end{IEEEeqnarray*} 
